...
from userbot import tgclient, USER_MODULES, ALL_MODULES, OS, MODULE_DESC, MODULE_DICT
...
async def superuser(command):
    ...
    if cmd_args[0] == "disclaimer":
        ...
        if modName in USER_MODULES:
            ...
            time.sleep(1)
            await command.edit("User module uninstalled successfully! Reboot recommended!")
            return
        else:
            await command.edit("`Uninstalling system module...`")
            os.remove(MODULES_DIR + modName + ".py")
            time.sleep(1)
            await command.edit("System module uninstalled successfully! Reboot recommended!")
            return
    else:
